...
import com.jone.record.entity.vo.LoginVo;
    ...
    public void findByLoginName(HttpServletRequest request, HttpServletResponse response, @RequestBody LoginVo object) {
        ...
        try {
            if (StringUtils.isEmpty(object.getLoginName())) {
                ...
                bd.setCode(-1);
            } else if (StringUtils.isEmpty(object.getCode())) {
                bd.setMessage("请填写验证码");
                bd.setCode(-1);
            } else if (!imageCode.toUpperCase().equals(object.getCode().toUpperCase())) {
                bd.setMessage("验证码错误");
                bd.setCode(-1);
            } else {
                user = userService.findByLoginNameOrMobile(object.getLoginName());
                if (null == user) {
                    bd.setMessage("用户不存在");
                    bd.setCode(-1);
                } else {
